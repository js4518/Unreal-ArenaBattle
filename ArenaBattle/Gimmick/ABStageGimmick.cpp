...
AABStageGimmick::AABStageGimmick()
{
	...
	for (FName GateSocket : GateSockets)
	{
		...
		Gate->SetupAttachment(Stage, GateSocket); // 소켓 이용하여 위치 지정
		...
		GateTrigger->ComponentTags.Add(GateSocket); // 소켓 이름으로 컴포넌트 태그 지정
...
void AABStageGimmick::SpawnRewardBoxes()
{
	for (const auto& RewardBoxLocation : RewardBoxLocations) {
		FTransform WorldSpawnTransform(GetActorLocation() + RewardBoxLocation.Value + FVector(0.f, 0.f, 30.f));
